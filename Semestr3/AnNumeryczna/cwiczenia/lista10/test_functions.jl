...
function chebyshevext(n)
    return [cospi(k/n) for k = 0:n]
end

function integral(f, a, b, n = 1000, T = BigFloat)
    h = (b-a)/T(n)
    x = a + h
    S = T(0)
    while(x <= b)
...
function scalarProd1(f, g, T = BigFloat)
    return integral(x->f(x)*g(x), -T(1), T(1), 10000)
end

function scalarProd2(f, T = BigFloat)
    return integral(x->f(x)*sqrt(1/(T(-625)*x^6 + T(575)*x^4 + T(49)*x^2 + 1)), -T(1), T(1), 10000)
end
